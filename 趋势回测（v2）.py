...
class Backtest:
    def __init__(self, data):
        self.data = data
        self.data['Daily_MA'] = self.data['Close'].rolling(window=20).mean()
        self.data['Weekly_MA'] = self.data['Close'].rolling(window=100).mean()
        self.data['Monthly_MA'] = self.data['Close'].rolling(window=200).mean()
            ...
            try:
                for i in range(1, len(self.data)):
                    ...
                    if self.data['Signal'].iloc[i] == 1 and self.data['Signal'].iloc[i-1] != 1:
                        # Buy signal
                        price = float(self.data['Close'].iloc[i].iloc[0])
                        ...
                        print(f"Buy: Date: {self.data.index[i].date()}, Price: {price:.2f}, Cash: {self.cash:.2f}, Shares: {self.shares}, Portfolio Value: {self.portfolio_value:.2f}, Return: {profit:.2%}")
                        f.write(f"{self.data.index[i].date()}, {price:.2f}, {self.cash:.2f}, {self.portfolio_value:.2f}, Buy, {self.shares}\n")
                        ...
                        # 计算每次交易的收益率
                        if self.shares != 0:
                            prev_price = float(self.data['Close'].iloc[i-1].iloc[0])
                        ...
                        print(f"Sell: Date: {self.data.index[i].date()}, Price: {price:.2f}, Cash: {self.cash:.2f}, Shares: 0, Portfolio Value: {self.portfolio_value:.2f}, Return: {profit:.2%}")
                        f.write(f"{self.data.index[i].date()}, {price:.2f}, {self.cash:.2f}, {self.portfolio_value:.2f}, Sell, 0\n")

                # Calculate final portfolio value
                final_price = float(self.data['Close'].iloc[-1].iloc[0])
                ...
                f.write(f"{self.data.index[-1].date()}, {final_price:.2f}, {self.cash:.2f}, {final_portfolio_value:.2f}, Final, {self.shares}\n")
